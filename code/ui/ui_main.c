...
#ifndef WESTERN3D
static char quake3worldMessage[] = "Visit our discord https://discord.gg/hUqegqKmUx";
...
static void UI_RunMenuScript(char **args) {
	...
	if (String_Parse(args, &name)) {
		...
		} else if (Q_stricmp(name, "Western3D") == 0) {
...
// Tequila comment: SkirmishStart implemented with StartServer in SG
			...
			trap_Cvar_Set("ui_sv_ws3d_version", Info_ValueForKey( serverinfo, "ws3d_version" ));
		}
#endif
		else {
...
static void UI_BuildServerDisplayList(qboolean force) {
	...
	if (len == 0) {
#ifndef WESTERN3D
		strcpy(uiInfo.serverStatus.motd, "Welcome to Western3D Arena!");
#else
	...
	for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		for (j=0; j<numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			if (Q_stricmpn(skinname, "icon_", 5) == 0)
			{
				...
				Com_Printf("Registering Western3D model: models/wq3_players/%s/%s\n",dirptr,skinname);
...
#if defined WESTERN3D && defined Q3_VERSION
	Com_Printf( "UI version: %s, %d, %s\n", Q3_VERSION, WS3D_RELEASE, __TIME__ );
#endif
