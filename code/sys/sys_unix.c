...
#if defined WESTERN3D && defined DEDICATED
uid_t saved_euid;
#endif
...
#define PREFIX /usr/share/games
	...
	// Let's also try ~/WESTERN3D/ - just for kicks
	if ((hp = getenv("HOME")) != NULL) {
		...
		// Let's also try ~/WESTERN3D/ - just to support default
		// folder set from the WESTERN3D 1.0 zip file
		Q_strncpyz(homedir, hp, sizeof(homedir));
